...
# X is the actual input value
# xmin and xmax are the range of values
# a and b are desired range
def normalize(X, xmin, xmax, a, b):
    x_xmin_delta = float(X-xmin)
    b_a_delta = float(b-a)
    xmax_xmin_detla = float(xmax-xmin)
    return a+(x_xmin_delta*b_a_delta/xmax_xmin_detla)
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            while True:
                ...
                controllerState.rightBumper = controller.rightBumper()
                controllerState.leftBumper = controller.leftBumper()

                controllerState.normalizeRightY = normalize(
                    controller.rightY(), -32768, 32767, -1, 1)
                controllerState.normalizeLeftY = normalize(
                    controller.leftY(), -32768, 32767, -1, 1)
                # controller.refresh()
